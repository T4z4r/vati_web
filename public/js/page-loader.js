(() => {
    ...
    document.addEventListener('click', event => {
        ...
        if (!link || link.hasAttribute('download') || link.hasAttribute('data-no-loader') ||
            link.hasAttribute('data-confirm') || link.target === '_blank') return;

        const url = new URL(link.href, window.location.href);
        if (url.origin !== window.location.origin || !['http:', 'https:'].includes(url.protocol) ||
            url.href === window.location.href || url.hash && url.pathname === window.location.pathname ||
            /\/(?:export|download)(?:\/|$)/i.test(url.pathname)) return;
