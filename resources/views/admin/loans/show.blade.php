...
<div class="grid-2"><div><div class="card"><div class="card-head"><h2>Repayment schedule</h2><span>{{ $loan->installments->where('status','paid')->count() }} / {{ $loan->number_of_installments }} paid</span></div><div class="table-wrap"><table><thead><tr><th>#</th><th>Due date</th><th>Due</th><th>Paid</th><th>Status</th></tr></thead><tbody>@forelse($loan->installments as $item)<tr><td>{{ $item->installment_number }}</td><td>{{ $item->due_date->format('d M Y') }}</td><td class="money">{{ number_format($item->total_due) }}</td><td class="money">{{ number_format($item->total_paid) }}</td><td><span class="badge {{ $item->status }}">{{ str_replace('_',' ',$item->status) }}</span></td></tr>@empty<tr><td colspan="5" class="empty">Schedule will appear after disbursement.</td></tr>@endforelse</tbody></table></div></div><br>
