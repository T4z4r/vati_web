...
@section('content')
    <div class="page-head">
        ...
        </div><a class="btn btn-secondary" href="{{ route('admin.loan-products.index') }}"><span class="ph ph-arrow-left" aria-hidden="true"></span> {{ __('Back') }}</a>
    </div>
            ...
            <div class="form-grid"><label>{{ __('Product name') }}<input name="name"
                ...
                <label>
                    {{ __('Minimum amount (TZS)') }}<input type="number" name="minimum_amount"
                        ...
                        value="{{ old('processing_fee_percentage', $product->processing_fee_percentage ?? 1) }}"
                        required></label><label>{{ __('Insurance (%)') }}<input type="number" step="0.0001"
                        name="insurance_percentage"
                        value="{{ old('insurance_percentage', $product->insurance_percentage ?? 1.5) }}"
