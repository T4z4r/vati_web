...
@php
    ...
    $utilizations = old('utilizations', $application->utilizations?->toArray() ?: [
...
<div class="page-head">
    ...
    <a class="btn btn-secondary" href="{{ $editing ? route('admin.loan-applications.show', $application) : route('admin.loan-applications.index') }}">← Back</a>
</div>

<form class="card" method="POST" action="{{ $editing ? route('admin.loan-applications.update', $application) : route('admin.loan-applications.store') }}">
    @csrf
    @if($editing) @method('PUT') @endif
    <div class="card-body">
        @if($editing)<div class="alert alert-danger">Saving financial changes invalidates the previous applicant consent and biometric confirmation. Capture them again before submission.</div>@endif
        <h3 class="section-title">Application terms</h3>
        <div class="form-grid">
            <label>Member
                @if($editing)<input type="hidden" name="member_id" value="{{ $application->member_id }}">@endif
                <select name="{{ $editing ? 'member_display' : 'member_id' }}" required @disabled($editing)>
                    <option value="">Select active member</option>
                    @foreach($members as $member)
                        <option value="{{ $member->id }}" @selected((int) old('member_id', $selectedMember) === $member->id)>{{ $member->membership_number }} · {{ $member->first_name }} {{ $member->last_name }} · {{ $member->group->group_name }}</option>
                    @endforeach
                </select>
            </label>
            <label>Loan product
            ...
            <label class="full">Loan purpose<textarea name="loan_purpose" required>{{ old('loan_purpose', $application->loan_purpose) }}</textarea></label>
            <label class="full">Business summary<textarea name="business_summary" required>{{ old('business_summary', $application->business_summary) }}</textarea></label>
        </div>

        <h3 class="section-title" style="margin-top:25px">Income & expenditure assessment</h3>
        <div class="form-grid">
            <label>Core business income<input type="number" min="0" name="assessment[core_business_income]" value="{{ old('assessment.core_business_income', $assessment?->core_business_income ?? 0) }}" required></label>
            <label>Other income<input type="number" min="0" name="assessment[other_income]" value="{{ old('assessment.other_income', $assessment?->other_income ?? 0) }}"></label>
            <label>Business expenses<input type="number" min="0" name="assessment[business_expenses]" value="{{ old('assessment.business_expenses', $assessment?->business_expenses ?? 0) }}" required></label>
            <label>Household expenses<input type="number" min="0" name="assessment[household_expenses]" value="{{ old('assessment.household_expenses', $assessment?->household_expenses ?? 0) }}" required></label>
            <label>Existing external debt<input type="number" min="0" name="assessment[existing_external_debt]" value="{{ old('assessment.existing_external_debt', $assessment?->existing_external_debt ?? 0) }}"></label>
            <label class="full">Assessment comment<textarea name="assessment[assessment_comment]">{{ old('assessment.assessment_comment', $assessment?->assessment_comment) }}</textarea></label>
        </div>
        ...
        <div class="table-wrap">
            <table>
                <thead><tr><th>Purpose</th><th>Allocation (TZS)</th><th>Present asset value</th></tr></thead>
                <tbody>
                    @foreach($utilizations as $index => $utilization)
                        <tr>
                            <td><input name="utilizations[{{ $index }}][purpose]" value="{{ $utilization['purpose'] ?? '' }}" placeholder="e.g. Working capital"></td>
                            <td><input class="allocation" type="number" min="0" name="utilizations[{{ $index }}][allocation_amount]" value="{{ $utilization['allocation_amount'] ?? '' }}"></td>
                            <td><input type="number" min="0" name="utilizations[{{ $index }}][current_asset_value]" value="{{ $utilization['current_asset_value'] ?? 0 }}"></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="form-actions">
            <a class="btn btn-secondary" href="{{ $editing ? route('admin.loan-applications.show', $application) : route('admin.loan-applications.index') }}">Cancel</a>
            <button class="btn btn-primary">{{ $editing ? 'Save draft changes' : 'Create draft application' }}</button>
        </div>
    </div>
</form>
@endsection
