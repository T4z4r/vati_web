...
class AppVersionController extends Controller
{
        ...
        $data = $request->validate([
            'apk' => [
                ...
                function (string $attribute, mixed $value, Closure $fail): void {
                    if ($value instanceof UploadedFile && ! $this->isAndroidPackage($value->getRealPath())) {
                        $fail('The uploaded file must be an Android application package (APK), not a zip archive.');
                    }
                },
            ],
    ...
    private function isAndroidPackage(string $path): bool
    {
        ...
        $isApk = $zip->locateName('AndroidManifest.xml') !== false && $zip->locateName('classes.dex') !== false;
        $isBundle = $zip->locateName('BundleConfig.pb') !== false;
        $zip->close();

        return $isApk || $isBundle;
    }
}
